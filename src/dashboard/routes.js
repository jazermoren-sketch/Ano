...
router.get('/servers/:guildId/settings',(req,res)=>res.json({guild:req.dashboardGuild,...getServerDashboard(req.params.guildId),protectionAdvanced:protectionConfig.getSettings(req.params.guildId),whitelist:protectionConfig.getWhitelist(req.params.guildId)}));
router.post('/servers/:guildId/protection',(req,res)=>{const allowed=['anti_spam','anti_links','anti_mass_mentions','anti_raid','anti_mass_actions'];const changes={};for(const key of allowed)if(typeof req.body[key]!=='undefined')changes[key]=req.body[key]?1:0;const basic=updateProtectionSettings(req.params.guildId,changes);const advanced={};for(const key of ['antiNuke','antiRaid'])if(typeof req.body[key]!=='undefined')advanced[key]=Boolean(req.body[key]);if(req.body.raidJoinLimit!==undefined)advanced.raidJoinLimit=Math.max(1,Math.min(100,Number(req.body.raidJoinLimit)||8));if(Array.isArray(req.body.whitelist))advanced.whitelist=req.body.whitelist.filter(x=>/^\d{17,20}$/.test(String(x))).slice(0,100);const current=Object.keys(advanced).length?protectionConfig.updateSettings(req.params.guildId,advanced):protectionConfig.getSettings(req.params.guildId);if(Array.isArray(req.body.whitelist))protectionConfig.setWhitelist(req.params.guildId,advanced.whitelist||[]);res.json({ok:true,protection:basic,advanced:current,whitelist:protectionConfig.getWhitelist(req.params.guildId)})});
